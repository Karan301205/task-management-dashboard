              ...
              <div className="bg-gray-50 rounded-lg p-3 mt-2">
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  {TAG_COLORS.map((color) => {
                    const bgColor = color.split(" ")[0].replace("bg-", "bg-")
                    return (
                      <button
                        key={color}
                        onClick={() => setNewTagColor(color)}
                        className={`w-6 h-6 rounded-full ${bgColor} border-2
                          ${newTagColor === color ? "border-gray-500 scale-110" : "border-transparent"}`}
                      ></button>
                    )
                  })}
                </div>
